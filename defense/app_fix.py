...
LLM Guard FIXED version — prompt injection blocked by DeBERTa classifier (Defense 1).
"""
import os
import re
import json
...
# Defense 1: LLM Guard prompt injection classifier (DeBERTa, CPU-only, no GPU required)
...
def run_ai_assistant_chat(username, user_id, user_message, history=None):
    ...
    # Defense 1: LLM Guard — block prompt injection before it reaches the model
    ...
    system_prompt = _ai_assistant_system_prompt()  # injection never reaches here
