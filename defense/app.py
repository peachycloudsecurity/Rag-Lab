"""
DevNotes AI Security Lab — minimal Flask app for LLM01 prompt injection labs.
Only the AI chat feature. No notes, admin, uploads, or RAG.
"""
import os
import time
import sqlite3
...
def ai_assistant():
    ...
    if not user:
        return redirect(url_for("login"))
    return render_template("ai_assistant.html", user=user, ai_status=get_ai_assistant_backend_status(), boot_time=APP_BOOT_TIME)
